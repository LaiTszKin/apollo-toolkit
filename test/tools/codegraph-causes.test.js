...
describe('codegraph error cause preservation', () => {
  ...
  it('SystemError re-wraps plain Error preserving original error via details.cause', () => {
    const originalError = new Error('Something went wrong inside a subcommand handler');
    ...
    const err = new SystemError(
      originalError instanceof Error ? originalError.message : 'Unknown error in codegraph',
      { cause: originalError },
    );

    // The original error is preserved in details.cause
    assert.ok(err.details?.cause, 'SystemError should preserve original error in details.cause');
    assert.strictEqual(err.details.cause, originalError, 'details.cause should be the original error from the subcommand');
  ...
  it('SystemError handles non-Error thrown value gracefully', () => {
    ...
    const thrownString = 'some string error';
    const err = new SystemError('Unknown error in codegraph', { cause: undefined });

    // When the thrown value is not an Error, cause should be undefined
    assert.strictEqual(err.details?.cause, undefined, 'details.cause should be undefined for non-Error thrown values');
  });
  ...
  it('SystemError message includes the original error message', () => {
    ...
    assert.ok(err.message.includes('Connection refused'), 'SystemError message should include the original message');
  ...
  it('handler re-throws SystemError and UserInputError as-is (does not double-wrap)', async () => {
    ...
    await assert.rejects(
      ...
      (err) => {
        assert.ok(err instanceof SystemError, 'should be SystemError (not double-wrapped)');
        assert.ok(err.message.includes('Unknown codegraph subcommand'), 'message should identify the unknown subcommand');
        return true;
      },
    );
  });
