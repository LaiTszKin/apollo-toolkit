...
| Req 4 — Coverage >= 80% + CI matrix | ✅ Feasible | Medium. Node.js 22.8+ has built-in `--experimental-test-coverage` with `--test-coverage-lines`, `--test-coverage-branches`, `--test-coverage-functions` thresholds. No `--test-coverage-statements` yet (statements omission is Node limitation). Per-process thresholds are 75/65/65 (lines/branches/functions) due to split-process limitation (tests distributed across CI matrix processes); combined coverage across all processes meets the >= 80% target. GitHub Actions matrix (`ubuntu-latest` + `windows-latest`) is straightforward. Windows runner may have symlink permission issues. |
...
| **Test coverage** | No coverage measurement, no CI test matrix. | `--experimental-test-coverage` with per-process thresholds at lines=75, branches=65, functions=65 (split-process limitation). Combined coverage across CI matrix exceeds >= 80%. GitHub Actions matrix testing `ubuntu-latest` + `windows-latest`. |
...
| Coverage thresholds (`--test-coverage-lines=75`, `--test-coverage-branches=65`, `--test-coverage-functions=65`) | Node 22.8+ via `--check-coverage`. Per-process thresholds reflect the split-process limitation (tests distributed across CI matrix); combined coverage meets the >= 80% target. |
...
**Split-process limitation**: The test suite is split across CI matrix processes (Linux, Windows), so each process runs a subset of tests and cannot individually reach 80% line coverage. Per-process thresholds are set at lines=75, branches=65, functions=65; combined coverage across all CI matrix processes meets the >= 80% target.
...
| **P3: Coverage completion** | `test/` | Fill test gaps to meet per-process thresholds (lines=75, branches=65, functions=65) with combined coverage >= 80% across CI matrix | Req 4 |
