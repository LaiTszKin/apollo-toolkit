...
| **Test coverage** | No coverage measurement, no CI test matrix. | `--experimental-test-coverage` with `--test-coverage-lines=80` threshold. GitHub Actions matrix testing `ubuntu-latest` + `windows-latest`. |
...
| Coverage thresholds (`--test-coverage-lines=80`) | Node 22.8+ via `--check-coverage` |
...
| **Built-in coverage instead of `c8`** | `c8` — supports `statements` threshold which Node built-in lacks; but adds a devDependency and CI install step | If we need statements-level threshold enforcement in the future, we add `c8` then. Lines threshold covers the 80% gate sufficiently. |
...
| **P3: Coverage completion** | `test/` | Fill test gaps to reach >= 80% line coverage | Req 4 |
