...
- T2.1 [x] **scripts/install_skills.sh `collect_skills()` 函數** — 將 `find "$REPO_ROOT" -mindepth 1 -maxdepth 1 -type d` 改為 `find "$REPO_ROOT/skills" -mindepth 1 -maxdepth 1 -type d`
  - Verify: 執行 `SKILL_DIR=$(bash scripts/install_skills.sh 2>&1 || true); echo "check manually"`

- T2.2 [x] **scripts/install_skills.sh `bootstrap_repo_if_needed()` / REPO_ROOT 檢測** — 檢查 curl/pipe 模式下 `find "$PWD" -mindepth 1 -maxdepth 1 -type d -exec test -f '{}/SKILL.md'` 的 REPO_ROOT 檢測邏輯，更新為正確識別 `skills/` 下的 SKILL.md
...
- T3.1 [x] **scripts/install_skills.ps1** — 尋找 PowerShell 腳本中對應 `collect_skills` 的路徑邏輯，更新為掃描 `skills/` 目錄
  - Verify: 閱讀腳本確認路徑更新正確
...
- T4.1 [x] **lib/cli.ts** — 搜尋 `SKILL.md` 或技能目錄相關路徑引用；無需更新（僅有說明文字）
  - Verify: `grep -rn "SKILL.md\|skill.*path\|skillDir\|skillsDir" lib/ bin/ --include="*.ts"` 確認所有引用已正確

- T4.2 [x] **lib/installer.ts** — 更新 `listSkillNames()`、`shouldCopyEntry()`、`resolveInstallSourcePath()` 的路徑邏輯
  - Verify: 同上

- T4.3 [x] **.gitignore** — 無需更新（`skills/` 目錄被 git 正確追蹤）
  - Verify: `git status` 確認 `skills/` 下檔案可被追蹤

- T4.4 [x] **執行測試** — 執行 `npm test` 確認全部通過；修復路徑相關問題（更新 installer.test.js、create-specs.test.js、atlas-*.test.js 的 fixture 和 import 路徑）
  - Verify: `npm test` 全部 184 測試通過
