...
export const yargsCommand = {
  command: 'search-logs [paths...]',
  describe: 'Search log lines by keywords or regex patterns with time filters',
  builder: (yargs: any) =>
    yargs
      .option('keyword', { type: 'string', describe: 'Keyword to search for (repeatable)' })
      .option('regex', { type: 'string', describe: 'Regex pattern to search for (repeatable)' })
      .option('mode', { type: 'string', choices: ['any', 'all'], describe: 'Match mode: any or all' })
      .option('ignore-case', { type: 'boolean', describe: 'Case-insensitive matching' })
      ...
      .option('before-context', { type: 'number', describe: 'Lines of context before each match' })
      .option('after-context', { type: 'number', describe: 'Lines of context after each match' })
  ...
  handler: async (argv: any) => {
    ...
    if (argv.keyword) {
      const kws = Array.isArray(argv.keyword) ? argv.keyword : [argv.keyword];
      for (const kw of kws) { args.push('--keyword', kw); }
    }
    if (argv.regex) {
      const rxs = Array.isArray(argv.regex) ? argv.regex : [argv.regex];
      for (const rx of rxs) { args.push('--regex', rx); }
    }
    if (argv.mode) { args.push('--mode', argv.mode); }
    if (argv.ignoreCase) { args.push('--ignore-case'); }
    ...
    if (argv.beforeContext != null) { args.push('--before-context', String(argv.beforeContext)); }
    if (argv.afterContext != null) { args.push('--after-context', String(argv.afterContext)); }
    if (argv.countOnly) { args.push('--count-only'); }
    await searchLogsHandler(args, { stdout: process.stdout, stderr: process.stderr });
  },
};
