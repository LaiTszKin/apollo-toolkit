...
export const yargsCommand = {
  command: 'review-threads <command> [args...]',
  describe: 'List or resolve GitHub PR review threads.',
  builder: (yargs: any) => yargs
    .command('list', 'List review threads', (yargs: any) => yargs
      .option('repo', { type: 'string', describe: 'GitHub repo (owner/repo)' })
      .option('pr', { type: 'number', describe: 'PR number' })
      .option('state', { type: 'string', describe: 'Thread state: unresolved|resolved|all', default: 'unresolved' })
      .option('output', { type: 'string', describe: 'Output format: table|json', default: 'table' })
      .strict(),
    )
    .command('resolve', 'Resolve review threads', (yargs: any) => yargs
      .option('repo', { type: 'string', describe: 'GitHub repo (owner/repo)' })
      .option('pr', { type: 'number', describe: 'PR number' })
      .option('thread-id', { type: 'string', describe: 'Thread ID to resolve' })
      .option('thread-id-file', { type: 'string', describe: 'JSON file with thread IDs' })
      .option('all-unresolved', { type: 'boolean', describe: 'Resolve all unresolved threads', default: false })
      .option('dry-run', { type: 'boolean', describe: 'Dry run mode', default: false })
      .strict(),
    )
    .demandCommand(1, 'Please specify a subcommand: list or resolve')
    .strict(),
  handler: async (argv: any) => {
    ...
    if (argv.pr) args.push('--pr', String(argv.pr));

    // list options
    if (argv.state && argv.state !== 'unresolved') args.push('--state', String(argv.state));
    if (argv.output && argv.output !== 'table') args.push('--output', String(argv.output));

    // resolve options
    if (argv.threadId) args.push('--thread-id', String(argv.threadId));
    if (argv.threadIdFile) args.push('--thread-id-file', String(argv.threadIdFile));
    if (argv.allUnresolved) args.push('--all-unresolved');
    if (argv.dryRun) args.push('--dry-run');

    await reviewThreadsHandler(args, {
