...
import { color, clearScreen, sleep, supportsAnimation, supportsColor } from './terminal.js';
...
export function buildWordmark({ colorEnabled }: { colorEnabled: boolean }): string {
...
export function buildSupportedTargetLines({ targets, colorEnabled }: { targets: TargetDefinition[]; colorEnabled: boolean }): string {
  const labelWidth = targets.reduce((max, t) => Math.max(max, t.label.length), 0);
  return targets.map((t) => (
    `  ${color(t.label.padEnd(labelWidth, ' '), '1', colorEnabled)} ${t.description}`
  )).join('\n');
}

export function buildWelcomeScreen({ version, colorEnabled, stage = 4, targets = [] }: WelcomeScreenOpts): string {
...
export async function animateWelcomeScreen({ output, version, env, targets = [] }: { output: NodeJS.WriteStream; version: string; env: NodeJS.ProcessEnv; targets?: TargetDefinition[] }): Promise<void> {
  if (!supportsAnimation(output, env)) return;

  const colorEnabled = supportsColor(output, env);
  for (const stage of [0, 1, 2, 3, 4]) {
    clearScreen(output);
    output.write(`${buildWelcomeScreen({ version, colorEnabled, stage, targets })}\n`);
    await sleep(stage === 0 ? 120 : 160);
  }
}

export function renderSelectionScreen({ output, version, cursor, selected, message, env, intro = 'Choose where Apollo Toolkit should copy managed skills.', choices, allValues }: SelectionScreenOpts): void {
  const colorEnabled = supportsColor(output, env);
