...
- 在 Commands 區塊開頭或附近加入前置條件：使用任何 `apltk` 或 `node dist/bin/apollo-toolkit.js` 命令前，先執行對應 `--help`，依 live CLI 指引選擇子命令與 flags。
- 若專案已安裝 `apltk codegraph`，在 Commands 區塊中用一句話描述它是代碼庫探索工具，不硬編碼完整子命令列表；要求 agent 先執行 `apltk codegraph --help`，再根據子命令 help 選擇合適的探索命令。
